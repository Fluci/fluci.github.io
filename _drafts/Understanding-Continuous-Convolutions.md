...
1. First, it applies [Leapfrog integration](https://en.wikipedia.org/wiki/Leapfrog_integration) to the input.
...
2. Dynamic and static particles need to be reduced to feature vectors.
    ...
    $$ s_i = (\boldsymbol x_j, [\boldsymbol n_j]) $$
...
3. The integrated values, together with additional features, are then fed to the network.
    ...
    We expect the position correction to consider all particle interactions, 
    including the collision handling with static particles.
